...
async function serviceWalkthrough(context, defaultValuesFilename, serviceMetadata) {
  ...
  if (!await amplify.confirmPrompt.run('Do you want a guided schema creation?')) {
    ...
    await context.amplify.executeProviderUtils(context, 'awscloudformation', 'compileSchema', { resourceDir, parameters, noConfig: true, authConfig });
...
async function askSecurityQuestions(context, parameters) {
  ...
  for(let i=0; i<additionalProvidersAnswer.authType.length; i++) {
  ...
  if (authConfig.defaultAuthentication.authenticationType === 'AMAZON_COGNITO_USER_POOLS' || additionalUserPoolProvider) {
    const userPoolId = authConfig.userPoolConfig ?
                        authConfig.userPoolConfig.userPoolId :
                        additionalUserPoolProvider.userPoolConfig.userPoolId;
...
async function askApiKeyQuestions() {
  ...
  let apiKeyConfig = await inquirer.prompt(apiKeyQuestions);
...
async function askOpenIDConnectQuestions() {
  ...
  let openIDConnectConfig = await inquirer.prompt(openIDConnectQuestions);
...
function validateDays(input) {
  ...
  const days = isValid ? parseInt(input) : 0;
